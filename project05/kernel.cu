...
{
	unsigned int wgNumber = blockIdx.x;							//work group number
	unsigned int wgDimension = blockDim.x;
	unsigned int threadNum = threadIdx.x;
	unsigned int gid = wgNumber*wgDimension + threadNum;		//thread number
...
main(int argc, char* argv[])
{
	...
	fprintf(stderr, "BLOCK SIZE : %10d | Number of Trials = %10d, MegaTrials/Second = %10.4lf\n", BLOCKSIZE, NUMTRIALS, megaTrialsPerSecond);
