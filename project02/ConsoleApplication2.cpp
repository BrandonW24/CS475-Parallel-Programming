...
#pragma omp parallel for reduction(+:volumeSum) shared(fullTileArea)				//Volume reduction and sharing full tile area
...
//				printf("Corner Tile detected : %f iu: %d\tiv: %d\tHeight: %f\t \n ", quarterTileArea/4, iu, iv, z);
				volumeSum += (fullTileArea /4) * z;
			}
			else {
//				printf("Edge Tile detected : %f iu: %d\tiv: %d\tHeight: %f\t \n ", halfTileArea / 2, iu, iv, z);
				volumeSum += (fullTileArea/2) * z;
			}

		}
		double timer1 = omp_get_wtime();
		double megaHeightperformance = (double)NUMNODES * (double)NUMNODES / (timer1-timer0)/1000000.;
