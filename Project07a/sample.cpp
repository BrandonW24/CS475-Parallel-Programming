...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Brandon Withington" };
...
const int NUM_PARTICLES = 8192*1024;
...
InitLists( )
{
	...
	// ****************************************
		...
		glColor3f( 1, 1, 1. );	// 0. <= r,g,b <= 1.
		glPushMatrix( );
			glTranslatef( -135.f, -800.f, 0.f );
	...
	// do this for yourself...
		glColor3f(0, .3f, .4f);	// 0. <= r,g,b <= 1.
		glPushMatrix();
		glTranslatef(-115.f, -640.f, 0.f);
		glutWireSphere(480.f, 80, 80);
		glPopMatrix();

		glColor3f(.7f, .3f, .4f);	// 0. <= r,g,b <= 1.
		glPushMatrix();
		glTranslatef(-45.f, -4000.f, 0.f);
		glutWireSphere(2000.f, 45, 45);
		glPopMatrix();
